...
/**
 * Student notification endpoint.
 *
 * GET  ?action=list          -> { ok, unread, items: [...] }
 * POST action=mark_all       -> { ok, unread: 0 }
 * POST action=mark_read&announcement_id=N -> { ok, unread }
 *
 * JSON-only — the navbar dropdown consumes it via fetch(). CSRF is
 * required on POST actions (GET is a safe read).
 */
...
try {
    ...
    /* GET: list + unread count in one round trip. */
    ...
    error_log('student/notifications.php failed: ' . $e->getMessage());
