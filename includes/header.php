...
$extraHead = $extraHead ?? '';
$loadStudentDashboardCss = $loadStudentDashboardCss ?? false;
...
<html
  ...
  <head>
    <meta charset="utf-8" />
    <script>
      /* Restore persisted sidebar-collapsed state before CSS paints to avoid
         a flash of the expanded menu on pages where the user left it collapsed.
         Paired with the MutationObserver in includes/footer.php. */
      (function () {
        try {
          var DESKTOP_BP = 1200;
          if (window.innerWidth >= DESKTOP_BP && localStorage.getItem('clms-menu-collapsed') === '1') {
            document.documentElement.classList.add('layout-menu-collapsed');
          }
        } catch (e) { /* storage disabled: render expanded, not a bug */ }
      })();
    </script>
    <meta
      name="viewport"
    ...
    <link rel="stylesheet" href="<?php echo htmlspecialchars($clmsWebBase, ENT_QUOTES, 'UTF-8'); ?>/public/assets/css/dashboard-clean.css" />
<?php if ($loadStudentDashboardCss) : ?>
    <link rel="stylesheet" href="<?php echo htmlspecialchars($clmsWebBase, ENT_QUOTES, 'UTF-8'); ?>/public/assets/css/student-dashboard.css" />
<?php endif; ?>
    ...
    <script src="<?php echo htmlspecialchars($clmsWebBase, ENT_QUOTES, 'UTF-8'); ?>/public/assets/js/clms-notify.js"></script>
    <script src="<?php echo htmlspecialchars($clmsWebBase, ENT_QUOTES, 'UTF-8'); ?>/public/assets/js/clms-scroll-restore.js"></script>
    <?php echo $extraHead; ?>
  </head>
  <body>
