            ...
            if ($action === 'update_student') {
                ...
                if ($newPassword !== '') {
                    ...
                    $upd->execute([
                        ...
                        'sb' => $batchParam, 'ph' => $hash, 'id' => (int) $userId,
                    ]);
                } else {
                    $upd = $pdo->prepare(
                        'UPDATE users
...
if ($searchQuery !== '') { $studentsFormFilterParams['q'] = $searchQuery; }
if ($filterAccount !== '') { $studentsFormFilterParams['account'] = $filterAccount; }
if ($filterApproval !== '') { $studentsFormFilterParams['approval'] = $filterApproval; }
if ($filterBatch !== '') { $studentsFormFilterParams['batch'] = $filterBatch; }
if ($page > 1) { $studentsFormFilterParams['page'] = (string) $page; }
