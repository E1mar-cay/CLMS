...
<?php
    ...
    $exportRankUrl = $clmsWebBase . '/instructor/rankings.php?' . http_build_query($exportRankParams);
?>
                          <a class="btn btn-outline-secondary text-nowrap" href="<?php echo htmlspecialchars($exportRankUrl, ENT_QUOTES, 'UTF-8'); ?>">Export CSV</a>
                        </div>
                        <small class="text-muted d-block mt-2">Period limits which attempts count. Batch and assessment filters apply after scores are computed.</small>
                      </div>
                    </form>
                  </div>
                </div>
