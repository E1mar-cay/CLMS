...
$activeInstructorPage = 'student_activity';
...
$batchOptions = [];
if ($selectedCourseInScope && $selectedCourseId) {
...
<?php endforeach; ?>
                    ...
                    <p class="small text-muted mb-0 mt-2">Assessment counts are module quizzes passed (≥70%). <?php echo $totalModules > 0 ? 'This course has <strong>' . (int) $totalModules . '</strong> module assessment(s).' : ''; ?> Batch uses optional cohort labels on student accounts.</p>
                  </div>
                </div>

<?php if ($selectedCourseInScope && $courses !== []) : ?>
                <div class="card mb-4 border shadow-none" style="background: rgba(105, 108, 255, 0.08);">
                  <div class="card-body py-3 d-flex flex-wrap align-items-center justify-content-between gap-3">
                    <div>
                      ...
                      <small class="text-muted">Students <strong>in progress</strong> vs. everyone with recorded activity in this course. Filters only affect the table.</small>
                    </div>
...
<?php
  ...
  $prevUrl = $clmsWebBase . '/instructor/student_activity.php' . ($prevParams !== [] ? '?' . http_build_query($prevParams) : '');
?>
...
<?php
  ...
  $pageUrl = $clmsWebBase . '/instructor/student_activity.php' . ($pageParams !== [] ? '?' . http_build_query($pageParams) : '');
?>
...
<?php
  ...
  $nextUrl = $clmsWebBase . '/instructor/student_activity.php?' . http_build_query($nextParams);
?>
